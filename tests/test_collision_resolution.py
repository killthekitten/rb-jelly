...
from rekordbox_to_jellyfin import Playlist, PlaylistGenerator, Track, UniqueNameResolver
...
class TestUniqueNameResolver:
    ...
    def test_collision_resolution(self, resolver):
        ...
        third = resolver.get_unique_name(
            "Plugin ears//Hovercat"
        )  # Sanitizes to "Plugin earsHovercat"
        assert (
            third == "Plugin earsHovercat"
        )  # Different from first two, so no collision
    ...
    def test_empty_and_special_names(self, resolver):
        ...
        assert (
            len(whitespace_result) > 0
        ), "Whitespace should generate a non-empty result"
        ...
        for reserved in reserved_names:
            ...
            assert (
                result != reserved
            ), f"Reserved name '{reserved}' should be sanitized, got '{result}'"
...
class TestPluginEarsCollisionScenario:
    ...
    def test_plugin_ears_collision_scenario(self):
        ...
        assert (
            results[1] == "Plugin ears  Hovercat (1)"
        ), f"Second result should have suffix: {results[1]}"

        # Third should be different (pathvalidate handles // differently than single /)
        assert (
            "Plugin ears" in results[2] and "Hovercat" in results[2]
        ), f"Third result should contain expected parts: {results[2]}"
    ...
    def test_complex_hierarchy_with_collisions(self):
        ...
        test_scenarios = [
            ...
            (
                ...
                "Test/Playlist",
                "TestPlaylist (1)",
            ),  # Collision in Sessions context
            (
                "UKG",
                "Test*Playlist",
                "TestPlaylist",
            ),  # Same name, different context again
        ]
        ...
        for context, input_name, expected in test_scenarios:
            ...
            assert (
                result == expected
            ), f"Failed for {input_name} in {context}: got {result}, expected {expected}"
...
class TestExtremeCollisionScenarios:
    ...
    def test_extreme_collision_scenario(self):
        ...
        assert len(results) == len(
            set(results)
        ), f"All results should be unique: {results}"
        ...
        for i, result in enumerate(results[1:], 1):
            assert (
                result == f"TestName ({i})"
            ), f"Result {i} should be 'TestName ({i})', got '{result}'"
...
class TestIntegrationWithPlaylistGenerator:
    ...
    def test_playlist_structure_creation_with_collisions(self, temp_dir, crates_root):
        ...
        mock_path_converter.validate_and_convert_path.return_value = (
            "/data/music/test_file.mp3"
        )
        ...
        created = generator.create_playlist_structure(
            collision_playlists, mock_path_converter
        )
        ...
        assert (
            len(set(created_names)) == 2
        ), f"Created playlist names should be unique: {created_names}"
    ...
    def test_nested_playlist_collision_resolution(self, temp_dir, crates_root):
        ...
        created = generator.create_playlist_structure(
            nested_playlists, mock_path_converter
        )

        # Should create unique files in the Sessions folder
        assert len(created) == 2
        sessions_files = [
            name for name in created.keys() if name.startswith("Sessions/")
        ]
        ...
        assert (
            len(set(playlist_names)) == 2
        ), f"Playlist names in Sessions folder should be unique: {playlist_names}"
