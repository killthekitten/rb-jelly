...
from rekordbox_to_jellyfin import PathConverter, Playlist, PlaylistGenerator, Track
...
class TestPlaylistGenerator:
    ...
    def test_create_playlist_structure_generates_m3u_files(self, temp_dir, crates_root):
        ...
        playlists = [
            Playlist(name="Electronic", path="Electronic", tracks=tracks, children=[])
        ]
        ...
        generator.clean_output_directory()
        created_playlists = generator.create_playlist_structure(
    ...
    def test_create_playlist_structure_skips_invalid_tracks(
        self, temp_dir, crates_root
    ):
        ...
        playlists = [Playlist(name="Mixed", path="Mixed", tracks=tracks, children=[])]
    ...
    def test_create_playlist_structure_handles_empty_playlists(
        self, temp_dir, crates_root
    ):
        ...
        playlists = [Playlist(name="Empty", path="Empty", tracks=tracks, children=[])]
    ...
    def test_create_playlist_structure_flat_mode_with_nested_playlists(
        self, temp_dir, crates_root
    ):
        ...
        playlists = [
            ...
            Playlist(
                name="Favorites", path="", tracks=tracks3, children=[]  # Root level
            ),
        ]
        ...
        # Verify file content
        content = (output_dir / "Electronic - Deep House.m3u").read_text(
            encoding="utf-8"
        )
        assert "Artist 1 - Deep House Track" in content
        assert "/data/music/Electronic/Deep/track1.mp3" in content

    def test_create_playlist_structure_nested_mode_vs_flat_mode(
        self, temp_dir, crates_root
    ):
        ...
        playlists = [
            Playlist(
                name="Test Playlist", path="Genre/Subgenre", tracks=tracks, children=[]
            )
        ]
        ...
        created_nested = generator_nested.create_playlist_structure(
            playlists, path_converter
        )
        ...
        created_flat = generator_flat.create_playlist_structure(
            playlists, path_converter
        )
        ...
        nested_content = (
            output_dir_nested / "Genre" / "Subgenre" / "Test Playlist.m3u"
        ).read_text()
        flat_content = (
            output_dir_flat / "Genre - Subgenre - Test Playlist.m3u"
        ).read_text()

        assert nested_content == flat_content  # Same track content

    def test_create_playlist_structure_flat_mode_multiple_playlists(
        self, temp_dir, crates_root
    ):
        ...
        playlists = [
            Playlist(name="House", path="Electronic", tracks=tracks1, children=[]),
            Playlist(name="Alternative", path="Rock", tracks=tracks2, children=[]),
            Playlist(
                name="Favorites", path="", tracks=tracks3, children=[]  # Root level
            ),
        ]
