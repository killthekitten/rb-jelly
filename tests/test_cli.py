...
try:
    from cli import cli, load_config, validate_required_config, check_smb_config
except ImportError:
...
class TestConfigFunctions:
    ...
    def test_load_config_defaults(self):
        ...
        env_vars_to_clear = ['REKORDBOX_DB_PATH', 'CRATES_ROOT', 'OUTPUT_DIR', 
...
class TestCLICommands:
    ...
    def teardown_method(self):
        ...
        env_vars_to_clean = ['REKORDBOX_DB_PATH', 'CRATES_ROOT', 'OUTPUT_DIR', 
                           'JELLYFIN_ROOT', 'SMB_SERVER', 'SMB_SHARE', 
                           'SMB_USERNAME', 'SMB_PASSWORD', 'LOG_LEVEL']
...
LOG_LEVEL=INFO"""
    ...
    def test_create_playlists_dry_run(self, mock_playlist_gen, mock_path_conv, mock_extractor):
    ...
    @patch('cli.PathConverter')  
    @patch('cli.PlaylistGenerator')
    def test_create_playlists_normal_mode(self, mock_playlist_gen, mock_path_conv, mock_extractor):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            try:
                ...
                mock_playlist_gen_instance.create_playlist_structure.return_value = {"Test": "test.m3u"}
    ...
    def test_sync_files_command(self, mock_smb, mock_path_conv, mock_extractor):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            try:
                ...
                mock_smb_instance.check_and_sync_files.return_value = (5, 3)  # 5 missing, 3 synced
...
SMB_PASSWORD=testpass"""
    ...
    def _create_mock_playlists(self):
        ...
        from rekordbox_to_jellyfin import Track, Playlist
    ...
    def test_create_playlists_no_playlists_found(self, mock_path_conv, mock_extractor):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            try:
                mock_extractor_instance = Mock()
                mock_extractor_instance.connect.return_value = True
                mock_extractor_instance.extract_playlists.return_value = []  # No playlists
                mock_extractor.return_value = mock_extractor_instance
