                ...
                for song in rb_playlist.Songs:
                    if hasattr(song, 'Content') and song.Content:
                        ...
                        track = Track(
                            ...
                            file_path=Path(content.FolderPath) / content.Filename if content.FolderPath and content.Filename else Path(),
                ...
                for track_key in track_keys:
                    ...
                    if track_data and hasattr(track_data, 'Location'):
                        track = Track(
                            ...
                            file_path=Path(track_data.Location).resolve() if track_data.Location else Path(),
...
class PlaylistGenerator:
    ...
    def create_playlist_structure(self, playlists: List[Playlist], path_converter: PathConverter) -> Dict[str, str]:
        ...
        for playlist in playlists:
            ...
            if valid_tracks:
                ...
                self.logger.info(f"Created playlist: {m3u_path} with {len(valid_tracks)} tracks")
            else:
...
class SMBSyncManager:
    ...
    def check_and_sync_files(self, tracks: List[Track], path_converter: PathConverter) -> Tuple[int, int]:
...
def main():
    ...
    if smb_manager and smb_manager.connect():
        ...
        logger.info(f"Sync completed: {synced_count}/{missing_count} missing files synced")
    else:
        if smb_manager:
            logger.error("Failed to connect to NAS. Playlist files created but sync skipped.")
        else:
            logger.info("SMB configuration not provided. Playlist files created but sync skipped.")
