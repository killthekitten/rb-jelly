...
try:
    ...
    click.echo(
        click.style(
            "Error: python-dotenv not installed. Run: pip install python-dotenv",
            fg="red",
        )
    )
    sys.exit(1)
...
@cli.command()
@click.option(
    "--check-only", is_flag=True, help="Only check for missing files, do not sync"
)
@click.pass_context
def sync_files(ctx, check_only):
    ...
    if not check_smb_config(config):
        click.echo(
            click.style(
                "❌ SMB configuration incomplete. Please check your .env file.",
                fg="red",
            )
        )
        click.echo(
            "Required variables: SMB_SERVER, SMB_SHARE, SMB_USERNAME, SMB_PASSWORD"
        )
        sys.exit(1)
    ...
    if check_only:
        click.echo(
            click.style(
                "🔍 CHECK ONLY MODE - No files will be synced", fg="yellow", bold=True
            )
        )
    ...
    click.echo(
        click.style(
            f"✅ Found {len(all_tracks)} tracks across {len(playlists)} playlists",
            fg="green",
        )
    )
    ...
    if check_only:
        ...
        click.echo(
            click.style(f"📊 Found {missing_count} missing files on NAS", fg="cyan")
        )
    else:
        ...
        if synced_count == missing_count:
            click.echo(
                click.style(
                    f"✅ Successfully synced all {synced_count} missing files",
                    fg="green",
                )
            )
        else:
            click.echo(
                click.style(
                    f"⚠️  Synced {synced_count}/{missing_count} missing files",
                    fg="yellow",
                )
            )
...
def full_migration(ctx, output_dir, skip_sync, flat):
    ...
    if not skip_sync and not has_smb_config:
        click.echo(
            click.style(
                "⚠️  SMB configuration incomplete. File sync will be skipped.",
                fg="yellow",
            )
        )
        skip_sync = True
...
def setup():
    ...
    if env_path.exists():
        if not click.confirm(
            click.style("⚠️  .env file already exists. Overwrite?", fg="yellow")
        ):
    ...
    if not example_path.exists():
        click.echo(
            click.style(
                "❌ .env.example file not found. Cannot proceed with setup.", fg="red"
            )
        )
        return
    ...
    source_type = click.prompt(
        "Choose source type", type=click.Choice(["database", "xml"]), default="database"
    )
    ...
    click.echo(
        click.style(
            "4. NAS/SMB Settings (optional - press Enter to skip)", fg="cyan", bold=True
        )
    )
    smb_server = click.prompt(
        "SMB server IP or hostname", default="", show_default=False
    )
...
"""
    ...
    click.echo(
        "  1. "
        + click.style("python cli.py config-check", fg="cyan")
        + " - Verify your configuration"
    )
    click.echo(
        "  2. "
        + click.style("python cli.py create-playlists --dry-run", fg="cyan")
        + " - Test playlist creation"
    )
    click.echo(
        "  3. "
        + click.style("python cli.py create-playlists", fg="cyan")
        + " - Create playlists"
    )
...
def config_check():
    ...
    if not env_exists:
        ...
        click.echo(
            click.style(
                "💡 Tip: Run 'python cli.py setup' to create configuration", fg="yellow"
            )
        )
        return
    ...
    click.echo(
        f"📁 Crates Root: {click.style(crates_status, fg=crates_color)} ({config['CRATES_ROOT']})"
    )
    ...
    if crates_valid and rekordbox_valid:
        ...
        if smb_complete:
            click.echo(
                "  • "
                + click.style("python cli.py sync-files --check-only", fg="cyan")
                + " - Check missing files"
            )
            click.echo(
                "  • "
                + click.style("python cli.py full-migration", fg="cyan")
